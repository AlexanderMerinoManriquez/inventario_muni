...
# ── Personas (funcionarios / usuarios sistema) ────────────────────────────────
...
def _normalizar_persona(item: dict) -> dict | None:
    ...
    nombre = _texto(item.get("nombre") or item.get("nombre_usuario"))

    if id_persona is None or not nombre:
        return None

    return {"id": id_persona, "rut": rut, "nombre": nombre}
...
def cargar_funcionarios() -> list[dict]:
    return _cargar_personas_api(API_FUNCIONARIOS_URL, API_SOURCE_FUNCIONARIOS)


def cargar_usuarios_sistema() -> list[dict]:
    return _cargar_personas_api(API_USUARIOS_SISTEMA_URL, API_SOURCE_USUARIOS_SISTEMA)
...
def _normalizar_departamento(item: dict) -> dict | None:
    """
    La API devuelve: {"id": 28, "oficina": "1ER JPL"}
    También soporta variantes históricas con otros nombres de campo.
    """
    ...
    return {"id": id_departamento, "nombre": nombre}


def cargar_departamentos() -> list[dict]:
